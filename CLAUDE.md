...
The `:stockholm-transport` JS webpack output is renamed to `commonjs2` and emitted under `build/js/packages/StockholmTransport-stockholm-transport/` (the gradle output dir name reflects both the XCFramework name and the Maven artifactId; the Node demo and `Dockerfile` reference that exact path).
...
- `demo/node-api/` — Express server in `server.js` that imports the JS package and exposes one HTTP endpoint per feature module. It calls `viewModel.subscribe(...)` and resolves once `state.isLoading` is false, then calls `viewModel.onCleared()`. After Option C it also exposes `/modules/active-trips` driven by `TripSelectionViewModel`. Also sends permissive CORS headers so the browser SPA can reach the `/v1` passthrough cross-origin.
- `demo/spa-bootstrap/` — Vite + React + TypeScript browser SPA consuming the **same** `StockholmTransport-stockholm-transport` JS package via the same `file:` dep as the Node demo. No business logic: each route uses `useStockholmTransport(factory, loader)` (in `src/hooks/useStockholmTransport.ts`), which wraps the library's `subscribe`/`onCleared`. The `/leak` route is the memory-leak demo (slide 18). Run `./sl publish` first, then `cd demo/spa-bootstrap && npm install && npm run dev`. TS types come from the hand-written `src/types/kmp.d.ts` shim until the generated package gains a `types` field.
